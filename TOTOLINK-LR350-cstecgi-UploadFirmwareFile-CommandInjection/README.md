# TOTOLINK LR350 cstecgi.cgi UploadFirmwareFile Command Injection
...
The `cstecgi.cgi UploadFirmwareFile` request is as follows: After receiving `FileName`, it uses `sprintf` to concatenate commands without checking or filtering shell metacharacters, and executes them with `doSystem`, causing a command injection vulnerability.
